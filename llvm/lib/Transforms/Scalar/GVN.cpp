...
// Disable Partial Redundancy Elimination for Testing
...
// ADD 'lvn' and 'svn' command-line options
static cl::opt<bool> GVNEnableLVN(
        "lvn", cl::init(false), cl::Hidden,
        cl::desc("Perform Local Value Numbering only"));

static cl::opt<bool> GVNEnableSVN(
        "svn", cl::init(false), cl::Hidden,
        cl::desc("Perform Super Local Value Numbering only and output EBBs"
                 "for each function"));
...
PreservedAnalyses GVNPass::run(Function &F, FunctionAnalysisManager &AM) {
  ...
  Changed = runImpl(F, AC, DT, TLI, AA, MemDep, LI, &ORE,
...
void GVNPass::dumpExtendedBasicBlocks(Function &F) {
    ...
    // Create output filename with .ebb extension
    SmallString<255> outputFileName(inputFileName);
    sys::path::replace_extension(outputFileName, ".ebb");

    // Open output file for writing
    std::error_code errCode;
    raw_fd_ostream outputFile(outputFileName, errCode, sys::fs::OF_Text);

    if (errCode) {
        errs() << "Error opening file " << outputFileName << ": " << errCode.message() << "\n";
        return;
    }
...
bool GVNPass::iterateOnFunction(Function &F) {
  ...
  if (GVNEnableSVN) {
      ...
      for (const auto &EBB : EBBs) {
          ...
          // Clear Value Number Table before processing next EBB
          cleanupGlobalSets();
      }
      ...
      for (BasicBlock *BB : RPOT) {
        ...
        // If Local Value Numbering is enabled, clear Value Number Table
